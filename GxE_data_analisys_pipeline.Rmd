...
author: "Bart-Jan van Rossum (modified by: Luis Fernando Delgado)"
date: "`r Sys.Date()`"
output: 
  html_document:
    toc: true
...
## The statgenGxE package {.unnumbered}
...
```{r loadData}
data(dropsPheno)
# ?dropsPheno #> information about data
```
...
|:-------------------|:------------------------------|:--------------------|
| Environments correspond to trials                          | **trial** = trial + **genotype + genotype:trial**                                                                                                                                                                   |                              |
...
### Model fitted with all terms as fixed {.unnumbered}
...
### Model fitted with all terms as random {.unnumbered}
...
dropsVarComp <- gxeVarComp(TD = dropsTD, 
                           trait = "grain.yield", 
                           nestingFactor = "scenarioFull")
...
library(tidyverse)
predGeno %>% arrange(desc(predictedValue))
...
head(predGenoTrial)
predGenoTrial %>% arrange(desc(predictedValue))
```
...
plot(dropsAm, scale = 0.4, plotType = "AMMI2", 
     colorGenoBy = "geneticGroup", colGeno = c("red", "blue", "green", "orange"),
     colorEnvBy = "scenarioFull", colEnv = c("purple", "#196f3d", "#2e4053", "#7d6608", "#6e2c00")) 
```
...
plot(dropsAm, scale = 0.4, plotType = "AMMI2", plotConvHull = TRUE, 
     colorEnvBy = "scenarioFull", colEnv = c("purple", "#196f3d", "#2e4053", "#7d6608", "#6e2c00"))
```
...
plot(dropsAm, scale = 0.4, plotType = "AMMI2", colorEnvBy = "scenarioFull",
     rotatePC = "Mur13W", colEnv = c("purple", "#196f3d", "#2e4053", "#7d6608", "#6e2c00"))
```
...
```{r geMegaEnvPred, R.options=list(digits=5), eval=FALSE}
